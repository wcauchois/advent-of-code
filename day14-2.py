...
class Reindeer(object):
  ...
  def __repr__(self):
    return "{} at position {} with {} points{}".format(self.name, self.position, self.points, " (resting)" if self.resting else "")
...
for i in xrange(0, 2503):
  for r in reindeer:
    r.tick()
  one_leader = max(reindeer, key=lambda r: r.position)
  leaders = [r for r in reindeer if r.position == one_leader.position]
  for l in leaders:
    l.points += 1
print reindeer
winner = max(reindeer, key=lambda r: r.points)
